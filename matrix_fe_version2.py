...
#   Make dummies and run regression (no need to demean)
#   Mar-03-2022
#-------------------------------------------------------
import numpy as np 
import pandas as pd
...
k_theta_dum = theta_dummies.shape[1]
phi_dummies = pd.get_dummies(df["phi_j"])
k_phi_dum = phi_dummies.shape[1]

theta_dummies = theta_dummies.values
phi_dummies = phi_dummies.values

A = df[["intercept", "x1", "x2", "x3"]].values
A = np.concatenate((A, theta_dummies[:, 1:k_theta_dum], phi_dummies[:, 1:k_phi_dum]), axis=1)
...
fe_output.to_excel (r'fe_output_version2.xlsx', index = True, header=True)    
